 ...
 */
typedef struct {
  unsigned int bits;                           /* length in bits of modulus 模数长度*/
  unsigned char modulus[MAX_RSA_MODULUS_LEN];                    /* modulus 模数*/
  unsigned char exponent[MAX_RSA_MODULUS_LEN];           /* public exponent 公开指数 */
} R_RSA_PUBLIC_KEY;

typedef struct {
  unsigned int bits;                           /* length in bits of modulus 模数长度*/
  unsigned char modulus[MAX_RSA_MODULUS_LEN];                    /* modulus 模数*/
  unsigned char publicExponent[MAX_RSA_MODULUS_LEN];     /* public exponent 公开指数*/
  unsigned char exponent[MAX_RSA_MODULUS_LEN];          /* private exponent 私有指数*/
  unsigned char prime[2][MAX_RSA_PRIME_LEN];               /* prime factors 素数因数*/
  unsigned char primeExponent[2][MAX_RSA_PRIME_LEN];   /* exponents for CRT CRT指数*/
  unsigned char coefficient[MAX_RSA_PRIME_LEN];          /* CRT coefficient	CRT系数 */
} R_RSA_PRIVATE_KEY;
