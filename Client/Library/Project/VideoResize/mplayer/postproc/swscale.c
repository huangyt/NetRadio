...
/*
    ...
    http://svn.sourceforge.net/viewcvs.cgi/ffdshow/
*/
...
int CPUCount= 1;	// Dual core, multiple CPU, Hyperthreadning tecnology support
int FreeCPUCount= 1;
BOOL CPUCountChanged= FALSE;
static SwsThreadParam *stp;
...
static SwsFunc getSwsFunc(int cpu){

#ifdef RUNTIME_CPUDETECT
#if defined(ARCH_X86) || defined(ARCH_X86_64)
        // ordered per speed fasterst first
	if(cpu & SWS_CPU_CAPS_MMX2)
		return swScale_MMX2;
	else if(cpu & SWS_CPU_CAPS_3DNOW)
		return swScale_3DNow;
	else if(cpu & SWS_CPU_CAPS_MMX)
		return swScale_MMX;
        else
		return swScale_C;
	...
	if(params->cpu & SWS_CPU_CAPS_MMX2)
        {
                ...
                if(!c->canMMX2BeUsed && dstW >=srcW && (srcW&15)==0 && (params->methodLuma.method&SWS_FAST_BILINEAR) && (params->methodChroma.method&SWS_FAST_BILINEAR))
                {
                        if(params->debug&SWS_PRINT_INFO)
...
#define MAX_FUNNY_CODE_SIZE 10000
	...
	c->array_lumPixBuf= (int16_t***)memalign(4, CPUCount*sizeof(int16_t***));
	c->array_chrPixBuf= (int16_t***)memalign(4, CPUCount*sizeof(int16_t***));
	c->array_lumMmxFilter= (int32_t **)memalign(4, CPUCount*sizeof(int32_t**));
	c->array_chrMmxFilter= (int32_t **)memalign(4, CPUCount*sizeof(int32_t**));
	c->array_formatConvBuffer= (uint8_t **)memalign(4, CPUCount*sizeof(uint8_t**));
	stp= (SwsThreadParam *) malloc(CPUCount*sizeof(SwsThreadParam));
	hThread= (HANDLE *) malloc(CPUCount*sizeof(HANDLE));

	for(i=0; i<CPUCount; i++)
	{
		int j;
		c->array_lumPixBuf[i]=(int16_t**)memalign(4, c->vLumBufSize*2*sizeof(int16_t*));
		        for(j=0; j<c->vLumBufSize; j++)
			{
		                c->array_lumPixBuf[i][j]= c->array_lumPixBuf[i][j+c->vLumBufSize]= (uint16_t*)memalign(16, 8000);	/* align at 16 bytes for AltiVec */
				memset(c->array_lumPixBuf[i][j], 0, 8000);        //try to avoid drawing green stuff between the right end and the stride end
			}
		c->array_chrPixBuf[i]=(int16_t**)memalign(4, c->vChrBufSize*2*sizeof(int16_t*));
		        for(j=0; j<c->vChrBufSize; j++)
			{
		                c->array_chrPixBuf[i][j]= c->array_chrPixBuf[i][j+c->vChrBufSize]= (uint16_t*)memalign(16, 16000);
				memset(c->array_chrPixBuf[i][j], 64, 16000);        //try to avoid drawing green stuff between the right end and the stride end
			}
		c->array_lumMmxFilter[i]=(int32_t *)memalign(8, sizeof(int32_t)*4*(c->vLumFilterSize+1));
		c->array_chrMmxFilter[i]=(int32_t *)memalign(8, sizeof(int32_t)*4*(c->vChrFilterSize+1));
		c->array_lumMmxFilter[i][4*c->vLumFilterSize]=0;	// End mark of Loop in the ASM code.
		c->array_chrMmxFilter[i][4*c->vChrFilterSize]=0;
		c->array_formatConvBuffer[i]=(uint8_t*)memalign(32, 8000);
	}

        c->lumPixBuf= c->array_lumPixBuf[0];
        c->chrPixBuf= c->array_chrPixBuf[0];
...
void sws_freeContext(SwsContext *c){
        ...
        for(i=0; i<CPUCount; i++)
         {
          int j;
          if (c->array_lumPixBuf && c->array_lumPixBuf[i])
           {
            for(j=0; j<c->vLumBufSize; j++)
             {
              if(c->array_lumPixBuf[i][j]) free(c->array_lumPixBuf[i][j]);
              c->array_lumPixBuf[i][j]=NULL;
             }
            free(c->array_lumPixBuf[i]);
            c->array_lumPixBuf[i]=NULL;
           }
          if (c->array_chrPixBuf && c->array_chrPixBuf[i])
           {
            for(j=0; j<c->vChrBufSize; j++)
             {
              if(c->array_chrPixBuf[i][j]) free(c->array_chrPixBuf[i][j]);
              c->array_chrPixBuf[i][j]=NULL;
             }
            free(c->array_chrPixBuf[i]);
            c->array_chrPixBuf[i]=NULL;
           }
         }
        if (c->array_lumPixBuf) free(c->array_lumPixBuf);
        c->array_lumPixBuf= NULL;
        if (c->array_chrPixBuf) free(c->array_chrPixBuf);
        c->array_chrPixBuf= NULL;
        if (c->array_lumMmxFilter) free(c->array_lumMmxFilter);
        c->array_lumMmxFilter= NULL;
        if (c->array_chrMmxFilter) free(c->array_chrMmxFilter);
        c->array_chrMmxFilter= NULL;
        if (c->array_formatConvBuffer) free(c->array_formatConvBuffer);
        c->array_formatConvBuffer= NULL;
 ...
 *  Sometimes the caller knows itself or other application uses threads while swscaler is working.
 *  In such case, call decCPUCount when caller's thread starts and incCPUCount when it ends.
 */
