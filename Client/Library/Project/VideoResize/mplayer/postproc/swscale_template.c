...
static inline void RENAME(yuv2yuvX)(SwsContext *c, int16_t *lumFilter, int16_t **lumSrc, int lumFilterSize,
				    ...
				    uint8_t *dest, uint8_t *uDest, uint8_t *vDest, long dstW, long chrDstW, int32_t *lumMmxFilter, int32_t *chrMmxFilter)
{
#ifdef HAVE_MMX
        if(uDest != NULL)
        {
				...
				:: "r" (&c->redDither),
				"r" (uDest), "p" (chrDstW), "r" (chrMmxFilter)
				: "%"REG_a, "%"REG_d, "%"REG_S
                        );
				...
				:: "r" (&c->redDither),
				"r" (vDest), "p" (chrDstW), "r" (chrMmxFilter)
				: "%"REG_a, "%"REG_d, "%"REG_S
                        );
        }
...
static int RENAME(swScaleSub)(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
             int srcSliceH, uint8_t* dst[], stride_t dstStride[], int dstYstart, int dstYend, int16_t **lumPixBuf, int16_t **chrPixBuf,
	     int32_t *lumMmxFilter,int32_t *chrMmxFilter,uint8_t *formatConvBuffer)
{
...
//	lumMmxFilter[4*vLumFilterSize]=0;	// End mark of Loop in the ASM code.
//	chrMmxFilter[4*vChrFilterSize]=0;
		...
		if(dstFormat == IMGFMT_NV12 || dstFormat == IMGFMT_NV21){
			...
			if((dstY&chrSkipMask) || isGray(dstFormat)) uDest=vDest= NULL; //FIXME split functions in lumi / chromi
				...
				RENAME(yuv2yuvX)(c,
                                        vLumFilter+dstY*vLumFilterSize     , lumSrcPtr, vLumFilterSize,
					vChrFilter+chrDstY*vChrFilterSize, chrSrcPtr, vChrFilterSize,
					dest, uDest, vDest, dstW, chrDstW, lumMmxFilter, chrMmxFilter);
                        }
                }
		else
		{
...
//        c->lastInChrBuf= lastInChrBuf;
	return 0;	//dstY - lastDstY;
/*
 *	I, Haruhiko Yamagata, commented out the above.
 *	I do not understand why the code above exist.
 *	ffdshow seems to works without it.
 *	If you use this in different project, please take care of this part.
 */
}


DWORD WINAPI RENAME(swScaleThread)(SwsThreadParam *stp)		// Thread entry
{
	RENAME(swScaleSub)(stp->c, stp->src, stp->srcStride, stp->srcSliceY,
			stp->srcSliceH, stp->dst, stp->dstStride, stp->dstYstart, stp->dstYend, stp->lumPixBuf, stp->chrPixBuf,
			stp->lumMmxFilter,stp->chrMmxFilter,stp->formatConvBuffer);
	return 0;
}
             ...
             int srcSliceH, uint8_t* dst[], int dstStride[])
{
	...
	DWORD ThreadID;		// We don't need its value.
	int dstLines;
	int i;
	int aCPUCount;

	aCPUCount=FreeCPUCount;

	if (aCPUCount==1)
	{
		return RENAME(swScaleSub)(c,src,srcStride,srcSliceY,
			srcSliceH,dst,dstStride,srcSliceY,c->dstH, c->lumPixBuf, c->chrPixBuf,
			c->array_lumMmxFilter[0], c->array_chrMmxFilter[0], c->array_formatConvBuffer[0]);
	}
	else
	{
		stp[0].dstYstart= 0;
		dstLines= stp[0].dstYend= c->dstH/aCPUCount;
		for (i=0; i<aCPUCount-1; i++){
			stp[i].c= c;
			stp[i].src= src;
			stp[i].srcStride= srcStride;
			stp[i].srcSliceY= srcSliceY;
			stp[i].srcSliceH= srcSliceH;
			stp[i].dst= dst;
			stp[i].dstStride= dstStride;
			stp[i].lumPixBuf=c->array_lumPixBuf[i];
			stp[i].chrPixBuf=c->array_chrPixBuf[i];
			stp[i].lumMmxFilter=c->array_lumMmxFilter[i];
			stp[i].chrMmxFilter=c->array_chrMmxFilter[i];
			stp[i].formatConvBuffer=c->array_formatConvBuffer[i];
			hThread[i]=CreateThread(
				&sa,
				0,
				(LPTHREAD_START_ROUTINE) RENAME(swScaleThread), 
				&stp[i],
				0,
				&ThreadID
			);
			stp[i+1].dstYstart= stp[i].dstYstart+dstLines;
			stp[i+1].dstYend= stp[i].dstYend+dstLines;

		}
		RENAME(swScaleSub)(c,src,srcStride,srcSliceY,
			srcSliceH,dst,dstStride, stp[i].dstYstart, c->dstH, c->array_lumPixBuf[i], c->array_chrPixBuf[i], 
			c->array_lumMmxFilter[i], c->array_chrMmxFilter[i], c->array_formatConvBuffer[i]);
		for (i=0; i<aCPUCount-1; i++){
