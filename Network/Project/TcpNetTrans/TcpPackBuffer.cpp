...
BOOL _tcp_pack_header::IsValid(void) const
{
	...
	// 版本检查
	if(m_nPackVersion != TCP_PACK_VERSION)
		return FALSE;
...
/// 打包
int32_t CTcpPackBuffer::Pack(const char* szInBuffer, uint16_t nInBufferSize, 
...
/// 解包
int32_t CTcpPackBuffer::UnPack(const char* szInBuffer, uint16_t nInBufferSize, 
